...
namespace hoa{
  ...
  static void Optim2DInPhase_process(Optim2DInPhase * unit, int inNumSamples);
    
  static void Optim2DInPhase_Ctor(Optim2DInPhase * unit);
  
  static void Optim2DInPhase_Dtor(Optim2DInPhase * unit);

  static void Optim3DInPhase_process(Optim3DInPhase * unit, int inNumSamples);
  ...
  static void Optim2DMaxRe_Ctor(Optim2DMaxRe * unit){
    ...
    unit->optim = (Optim<Hoa2d,float>::MaxRe*)RTAlloc(unit->mWorld,sizeof(Optim<Hoa2d,float>::MaxRe(order)));
    ...
    unit->output = (float*)RTAlloc(unit->mWorld,sizeof(float)* unit->numberOfHarmonics * FULLBUFLENGTH);
  ...
  static void Optim2DMaxRe_process(Optim2DMaxRe * unit, int inNumSamples){
    ...
    for (int i = 0; i < inNumSamples; ++i){
      tempOptim->process(inputs + i * numHarmonics, outputs + i * numHarmonics);
    }
  ...
  static void Optim3DMaxRe_Ctor(Optim3DMaxRe * unit){
    ...
    unit->optim = (Optim<Hoa3d,float>::MaxRe*)RTAlloc(unit->mWorld,sizeof(Optim<Hoa3d,float>::MaxRe(order)));
  ...
  static void Optim2DInPhase_Ctor(Optim2DInPhase * unit){
    ...
    unit->optim = (Optim<Hoa2d,float>::InPhase*)RTAlloc(unit->mWorld,sizeof(Optim<Hoa2d,float>::InPhase(order)));
  ...
  static void Optim3DInPhase_Ctor(Optim3DInPhase * unit){
    ...
    unit->optim = (Optim<Hoa3d,float>::InPhase*)RTAlloc(unit->mWorld,sizeof(Optim<Hoa3d,float>::InPhase(order)));
